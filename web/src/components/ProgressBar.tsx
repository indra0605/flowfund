    ...
    <article className="card">
      <header className="card-head">
        <div className="card-title">
          ...
          <span className="muted small mono">#{campaign.id}</span>
        </div>
      ...
      {campaign.description ? (
        <p className="card-description clamp-2">{campaign.description}</p>
      ) : null}
      <div className="metric">
        <strong>{fmtStroops(campaign.raised)} XLM</strong>
        <span>{fmtStroops(campaign.goal)} XLM goal</span>
      </div>
      <ProgressBar value={p} />
      <div className="card-meta">
        <span className="muted small">{p.toFixed(1)}% funded</span>
        <span className="muted small">{timeLeft(campaign.deadline)}</span>
      </div>
      <div className="meta">
