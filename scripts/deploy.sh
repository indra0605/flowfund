#!/usr/bin/env bash
# Build + deploy the FlowFund Soroban contract.
#
...
#   ./scripts/deploy.sh --alias flowfund             # name the deployment (default: flowfund)
#
...
CONTRACT_ALIAS="flowfund"
CONTRACT_NAME="flowfund"
RESET="false"
INVOKE_FN=""
INVOKE_ARGS=()
...
DEPLOYMENTS_FILE="$ROOT_DIR/.soroban/deployments.json"
...
WASM_HASH="$(shasum -a 256 "$WASM_OPT" | awk '{print $1}')"
DEPLOY_TX_HASH="$(printf '%s\n' "$DEPLOY_OUT" | sed -n 's|.*explorer/.*/tx/\([[:alnum:]]\{64\}\).*|\1|p' | tail -n 1)"
LAB_URL="$(printf '%s\n' "$DEPLOY_OUT" | sed -n 's|.*\(https://lab\.stellar\.org[^[:space:]]*\).*|\1|p' | tail -n 1)"
...
cat > "$DEPLOYMENTS_FILE" <<JSON
{
  ...
  "source_account": "$ACCOUNT_ADDRESS",
